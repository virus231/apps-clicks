...
const Home = () => {
  const [list, setListing] = useState<Article[]>([]);
  ...
  const fetchTitles = async () => {
    try {
      ...
      const data = await response.json() as ITitle;

      setListing(data.titles)
          ...
          <Item>
            ...
            {!loading ? (
              list.map(({ title, id }, index) => (
                <h1 key={id} className="text-black">
                  {index + 1}. {title}
                </h1>
              ))
            ) : (
            ...
            <button
              onClick={() => fetchTitles()}
              className="mb-2 h-10 rounded-full border border-slate-200 px-6 font-semibold text-slate-700"
              type="button"
